...
const others =sellerKey? all.filter(
    (x)=>
    String(x.id)!== 
    String(item.id) && 
    String(x?.userId?? x?.user?.id?? x?.sellerId??x?.ownerId??"")===sellerKey)
    .slice(0,3) :[];


    const img=getImageUrl(item);
    const createdAt= item?.createdAt || item?.created_at || item?.data || null;
    ...
    return (
        <div className="space-y-10">
            ...
            <section>
                ...
                {others.length===0 ? (
                    <div className="text-gray-600">  No other item from this user.
                    </div>
                ) : (
